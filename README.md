...
- Uncomment and update the `Cities2_Location` variable in `MasterPlannedRadio.csproj`
- Run `make build`
...
You can leverage the helper script in `scripts/rename.csx` in order to replace "MasterPlannedRadio" with whatever you want to name your project. Usage:

```
$ dotnet script scripts\rename.csx "MasterPlannedRadio" "AnotherModIMade"
```

# Set license details

You'll need to update `LICENSE` with the correct details for `<Year>` and `<Author>`, and change "MasterPlannedRadio" to your mod name if you haven't already.
